...
email.addEventListener("input", function(){
    if (!checkEmail(email.value)) {
        emailError.textContent = "*Email deo dung dinh dang"
    }else {
...
password.addEventListener("input", function(){
    if (password.value.length < 8) {
        passwordError.textContent = "*Password Phai it nhat 8 ky tu =)))."
    }else if(password.value.length > 20){
        passwordError.textContent = "*Password Phai it hon 20 ky tu =)))."
    }else {
...
lgEmail.addEventListener("input", function(){
    if (!checkEmail2(lgEmail.value)) {
        lgEmailError.textContent = "*Dcmm viet Email deo dung"
    }else {
...
lgPassword.addEventListener("input", function(){
    if (lgPassword.value.length < 8) {
        lgPasswordError.textContent = "*Password Phai it nhat 8 ky tu =)))."
    }else if (lgPassword.value.length > 20){
        lgPasswordError.textContent = "*Password Phai it hon 20 ky tu =)))."
    }else {
...
function checkRequiredLg(inputArr2) {
    ...
    inputArr2.forEach(function(input){
        if (input.value.trim() === '') {
            showError2(input, `*${getFieldNameLg(input)} Dcmm dien thong tin day du`)
            isRequiredLg = true
        }else {
            showSuccess2(input)
        }
